#!/usr/bin/env python3
"""Single-file UCI chess engine using python-chess.

Features:
  - Negamax alpha-beta with Principal Variation Search (PVS)
  - Iterative deepening with aspiration windows
  - Quiescence search (captures, MVV-LVA ordered)
  - Transposition table (Zobrist-keyed, with bound flags)
  - Null-move pruning (with non-pawn-material guard)
  - Late move reductions (LMR)
  - Check extensions
  - Killer moves + history heuristic
  - Tapered PeSTO-style evaluation (material + piece-square tables)
  - Robust UCI loop with proper time management.
"""
...
def evaluate(board):
    """Tapered PeSTO-style evaluation. Returns score from side-to-move POV (cp)."""
    mg_w = 0
    mg_b = 0
    eg_w = 0
    eg_b = 0
...
class Searcher:
    ...
    def go(self, board, soft_time, hard_time, max_depth):
        self.nodes = 0
        self.stop = False
...
def compute_time(board, params):
    movetime = params.get("movetime")
    depth = params.get("depth")

    if depth is not None and movetime is None and params.get("wtime") is None and params.get("btime") is None:
        return 3600.0, 3600.0, depth

    if movetime is not None:
        mt = max(movetime / 1000.0 - 0.03, 0.01)
        return mt, mt, (depth if depth is not None else 64)

    wtime = params.get("wtime")
    btime = params.get("btime")
    winc = params.get("winc", 0) or 0
    binc = params.get("binc", 0) or 0
    movestogo = params.get("movestogo")

    if wtime is None and btime is None:
        return 5.0, 5.0, (depth if depth is not None else 64)

    my_time = wtime if board.turn else btime
    my_inc = winc if board.turn else binc
    if my_time is None:
        my_time = 60000
    if my_inc is None:
        my_inc = 0

    my_time_s = my_time / 1000.0
    my_inc_s = my_inc / 1000.0

    if movestogo and movestogo > 0:
        soft = my_time_s / max(movestogo, 1) + my_inc_s * 0.8
    else:
        soft = my_time_s / 30.0 + my_inc_s * 0.8

    safety = 0.05
    max_usable = max(my_time_s - safety, 0.01)
    hard = min(soft * 4.0, max_usable * 0.5)
    soft = min(soft, hard)
    if soft < 0.01:
        soft = 0.01
    if hard < soft:
        hard = soft
    return soft, hard, (depth if depth is not None else 64)
...
def parse_go(tokens):
    ...
    while i < len(tokens):
        t = tokens[i]
        if t in ("movetime", "wtime", "btime", "winc", "binc", "depth", "movestogo", "nodes"):
            if i + 1 < len(tokens):
                try:
...
def main():
    ...
    while True:
        ...
        try:
            if line == "uci":
                sys.stdout.write("id name SimpleOneShot 1.0\n")
                sys.stdout.write("id author SimpleOneShot\n")
                ...
                tokens = line.split()[1:]
                board = parse_position(board, tokens)
            elif line.startswith("go"):
                tokens = line.split()[1:]
                params = parse_go(tokens)
                ...
                if params.get("infinite"):
                    ...
                    soft, hard, max_depth = compute_time(board, params)

                move = searcher.go(board, soft, hard, max_depth)
                if move is None:
                    legal = list(board.legal_moves)
                    if legal:
                        move = legal[0]
            ...
            try:
                legal = list(board.legal_moves)
                if legal:
                    sys.stdout.write(f"bestmove {legal[0].uci()}\n")
                    sys.stdout.flush()
            except Exception:
                pass
